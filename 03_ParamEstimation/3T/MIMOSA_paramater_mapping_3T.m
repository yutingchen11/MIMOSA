...
T2s_all = zeros(size(img,1),size(img,2),size(img,3));
IE_all = zeros(size(img,1),size(img,2),size(img,3));
...
imagesc3d2(sq(T2std), s(sq(T2std))/2, 8, [180 180 180], [0 200]);colormap hot
...
parpool(min(ceil(total_cores*0.5), 10))
            ...
            for b = 1:length(b1_val)
                ...
                if num_vox_all > 0
                    ...
                    for t = 1:9
                        ...
                        if estimate_pd_map
                            [Mz_sim, Mxy_sim] = sim_MIMOSA_for3T_v2(TR, alpha_deg, esp, turbo_factor, res_map(:,1)*1e-3, res_map(:,2)*1e-3, num_reps, echo2use,TR_mte,esp_mte,TEs ,T2s_val(t2ss)*1e-3, gap_between_readouts, time2relax_at_the_end, b1_val(b), ie_to_use);
                        end
...
imagesc3d2(sq(T1_map), s(sq(T1_map(:,:,:)))/2, 1, [180 180 180], [0 3000]);colormap hot
imagesc3d2(sq(T2_map), s(sq(T1_map(:,:,:)))/2, 2, [180 180 180], [0 200]);colormap hot
imagesc3d2(sq(T2s_map), s(sq(T1_map(:,:,:)))/2, 3, [180 180 180], [0 200]);colormap hot
...
resgen = false; % Determine whether to use resolution generalization pipeline or to interpolate to 1 mm isotropic resolution
if resgen
...
imagesc3d2( x_para, s(x_para)/2+[26 0 10], 22, [180,180,180], [0 0.1])
imagesc3d2( x_dia, s(x_dia)/2+[26 0 10], 33, [180,180,180], [0 0.1])
