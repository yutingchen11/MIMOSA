...
thre_high   = 1.4;
thre_low    = 0.01;
num_b1_bins = 140; % default: 50  --> decrease to 20
...
signal = zeross([length(t1t2_lut_prune),7, length(T2s_val),  length(b1_val)]);
...
% for t2s = 1:length_t2s_val
    for b1 = 1:length_b1_val
        % for ie = 1:length_inv_eff
            cnt = cnt + 1;
            
            [Mz, Mxy] = sim_MIMOSA_for7T_v2(TR, alpha_deg, esp, turbo_factor, t1t2_lut_prune(:,1)*1e-3, t1t2_lut_prune(:,2)*1e-3, num_reps, echo2use,TR_mte,esp_mte,TEs, T2s_val(t2s)*1e-3, gap_between_readouts, time2relax_at_the_end, b1_val(b1), ie_lkp_prune(:,1));
...
dict = signal;
clear signal
save('dict/dict_mimosa_TE60p4_B1_140_v1.mat','dict','-v7.3')