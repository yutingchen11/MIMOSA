...
%%
load('csv_imports/traj_etl_115_meas_MID02225_FID205475_qalas_TE60ms_highres_v0_750um_R2x2.mat')

traj = traj(:,[3,1,2]); %[index ky_position kz_position]
nETL = 115;
nacq = 7;
...
t2prep = readmatrix('csv_imports/versedHS_NormMagPhase_mu5_beta400_8400us.txt');% inteparated in 7T
...
IRprep_pulse = mr.makeArbitraryRf((Re+Im*1i).', 750*pi/180, 'system',sys, 'dwell', 1e-5);% %reduce to 750 in 7T due tio SAR

% ############# define MGRE module paramater #############
esp_mte = 7;
TEs = [3:esp_mte:25] * 1e-3;%
TR_mte = 27.5e-3;
nechoes = length(TEs);
...
% % TEeff = 60ms, symetric for 8.4ms pulse
delay_pre_t2prep = 40e-3; 
delay_1_t2prep  =   3.4e-3;
delay_2_t2prep  =   25e-3 - 10e-3;   % x 3     
delay_3_t2prep  =   11.6e-3;
...
Re = text(:,1) .* cos(text(:,2));
Im = text(:,1) .* sin(text(:,2));
rf_acs = mr.makeArbitraryRf((Re+Im*1i).', flip_angle_acs*pi/180, 'system', sys, 'dwell', 1e-6);
...
for iZ = 1:nDummies
    ...
    % T2 prep pulse
    seq.addBlock(mr.makeDelay(delay_pre_t2prep));% use when T2 prep is not 100ms
    ...
    % FLASH readout 1
    ind_acq = 1;
    [rf_phase, rf_inc] = addAcq(seq, nETL, iZ, rf, adc, rfSpoilingInc, rf_phase, rf_inc, stepsZ, stepsY, gxPre, gx, gxSpoil, delay_TE, delay_TRinner, gyPre,gyReph,gzPre,gzReph,nacq,ind_acq,traj,nTR,useAdc);
    ...
    % MGRE Module
    ind_acq = 4;
    [rf_phase, rf_inc] = addAcq_mte(seq, nETL, iZ, rf, adc, rfSpoilingInc, rf_phase, rf_inc, stepsZ, stepsY, gxPre, gx, gxSpoil, delay_TE, delay_TRinner, gyPre,gyReph,gzPre,gzReph,TEs, gxFlyBack,delayTE_mte, delayTR_mte,nacq,ind_acq,traj,nTR,useAdc);
...
seq.write(['MIMOSA_T2prep8.4_750um_R4','.seq']);


%% calc duration
acs_duration = (Ndummy_acs + Ny_ref * Ny_ref) * TR_ref;

disp(['acs duration ', num2str(acs_duration), ' sec'])

scan_duration = (nDummies + nTR) * 5.7552;
...
function [rf_phase, rf_inc] = addAcq(seq, nETL, iZ, rf, adc, rfSpoilingInc, rf_phase, rf_inc, stepsZ, stepsY, ...
    gxPre, gx, gxSpoil, delay_TE, delay_TRinner, gyPre,gyReph,gzPre,gzReph,nacq,ind_acq,traj,nTR,useAdc)


for iY = 1:nETL

    % Calculate index for ky-kz look up table
    index = iY + (iZ-1)*nETL*5;% 
    
    disp(['acq_' num2str(ind_acq) ',index=',num2str(index)]);
    ...
    % Encoding
    seq.addBlock(gxPre, ...
        mr.scaleGrad(gyPre,double(-stepsY(index))), ...
        mr.scaleGrad(gzPre,double(-stepsZ(index))));    % Gz, Gy blips, Gx pre-winder
    ...
    seq.addBlock(gxSpoil, ...
        mr.scaleGrad(gyReph,double(stepsY(index))), ...
        mr.scaleGrad(gzReph,double(stepsZ(index))));    % -Gz, -Gy blips, Gx spoiler

    seq.addBlock(mr.makeDelay(delay_TRinner));  % wait until desired echo spacing
end
end
%##########
function [rf_phase, rf_inc] = addAcq_mte(seq, nETL, iZ, rf, adc, rfSpoilingInc, rf_phase, rf_inc, stepsZ, stepsY, ...
    gxPre, gx, gxSpoil, delay_TE, delay_TRinner, gyPre,gyReph,gzPre,gzReph,TEs, gxFlyBack,delayTE_mte, delayTR_mte,nacq,ind_acq,traj,nTR,useAdc)
    ...
    for c=1:length(TEs) % loop over TEs
        ...
        if c==1 
            ...
            seq.addBlock(gxFlyBack, ...
                mr.scaleGrad(gyReph,double(-(stepsY(index) - stepsY(ind_pre)))), ...
                mr.scaleGrad(gzReph,double(-(stepsZ(index)- stepsZ(ind_pre)))));    % Gz, Gy blips, Gx pre-winder
        end
